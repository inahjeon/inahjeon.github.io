...
# 외산 클라우드 없이 AI 플랫폼 제공하기: features, training, serving, and AI Suite. - 현동석님
...
- NLP 작업의 경우 API화 하여 사용
- 모델 서빙 시 inference 외 전/후 처리를 담당하는 서버 따로 사용

# 신호처리 이론으로 실용적인 스타일 변환 모델 만들기 (Better Faster Stronger Transfer) - 유재준님
...
# 문자인식(OCR), 얼마나 정확하지? (문자인식 성능을 정확하게 측정하는 방법) - 최찬규님
...
# 레이블링 조금 잘못돼도 괜찮아요: Clova가 레이블 노이즈 잡는 법 - 강재욱님
...
## 레이블링 바로잡는 AutoML (in project Khan)
...
# 자율주행 시뮬레이터를 개발하면서 경험한 한계점 및 활용 방안 - 홍준님
